...
(function () {
  ...
  let blocked = getCookie('al-blocked')
    ? getCookie('al-blocked').split(',')
    : [];
  let whiteList = getCookie('al-whiteList')
    ? getCookie('al-whiteList').split(',')
    : [];
  let blockGroups = safeJsonParse(getCookie('al-blockGroups')) || {}; // 使用安全解析
  let filterKeywords = getCookie('al-filterKeywords')
    ? getCookie('al-filterKeywords').split(',')
    : [];
  ...
  let statusBgColor =
    getCookie('al-statusBgColor') || 'linear-gradient(to right, #333, #222)';
  let menuOpacity = parseFloat(getCookie('al-menuOpacity')) || 0.9;
  let menuBgColor =
    getCookie('al-menuBgColor') ||
  ...
  let statusTextColor =
    getCookie('al-statusTextColor') ||
    (statusBgColor.includes('#333') || statusBgColor.includes('#222')
      ? '#ddd'
      : '#333');
  let statusTextBrightness =
    parseFloat(getCookie('al-statusTextBrightness')) || 1.0;
  ...
  let enableLoginCheck =
    getCookie('al-enableLoginCheck') !== undefined
      ? Boolean(getCookie('al-enableLoginCheck'))
      : true; // 新增：登录检测
  ...
  function updateStats(key) {
    stats[currentAccount] = stats[currentAccount] || {
      likes: 0,
      skips: 0,
      errors: 0,
    };
  ...
  function checkLoginStatus() {
    ...
    if (!isLoggedIn) {
      ...
      sendNotification(
        '登录过期',
        'QZone Praise Automator 检测到登录状态失效，请重新登录以继续。'
      );
  ...
  function setupMutationObserver() {
    const targetNode =
      document.querySelector('#feed_friend_list') || document.body; // 假设好友动态容器为#feed_friend_list，实际根据QQ空间调整
    if (!targetNode) {
    ...
    mutationObserver = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
          let hasNewLikes = Array.from(mutation.addedNodes).some(
            (node) =>
              node.querySelector && node.querySelector('.qz_like_btn_v3')
          );
          if (hasNewLikes) {
  ...
  function saveSettings() {
    try {
      ...
      if (autoReloginEl) {
        enableAutoRelogin = !!autoReloginEl.checked;
        setCookie(
          'al-enableAutoRelogin',
          enableAutoRelogin ? '1' : '',
          Number.MAX_SAFE_INTEGER
        );
      }
      ...
      if (statusBgColorEl) {
        statusBgColor = statusBgColorEl.value || statusBgColor;
      ...
      if (statusTextColorEl) {
        statusTextColor = statusTextColorEl.value || statusTextColor;
        setCookie(
          'al-statusTextColor',
          statusTextColor,
          Number.MAX_SAFE_INTEGER
      ...
      var statusTextBrightnessEl = document.getElementById(
        'al-statusTextBrightness'
      );
      if (statusTextBrightnessEl) {
        statusTextBrightness =
          parseFloat(statusTextBrightnessEl.value) || statusTextBrightness;
        setCookie(
          'al-statusTextBrightness',
          statusTextBrightness,
          Number.MAX_SAFE_INTEGER
      ...
      if (darkModeAutoEl) {
        darkModeAuto = !!darkModeAutoEl.checked;
        setCookie(
          'al-darkModeAuto',
          darkModeAuto ? '1' : '',
          Number.MAX_SAFE_INTEGER
  ...
  function resetSettings() {
    try {
      ...
      dailyLimit = 0;
      setCookie('al-dailyLimit', dailyLimit, Number.MAX_SAFE_INTEGER);
      smartLikeMode = 'normal';
      setCookie('al-smartLikeMode', smartLikeMode, Number.MAX_SAFE_INTEGER);
      select = false;
      setCookie('al-select', '', Number.MAX_SAFE_INTEGER);
      enableLoginCheck = true;
      setCookie('al-enableLoginCheck', '1', Number.MAX_SAFE_INTEGER);
      enableAutoRelogin = false;
      setCookie('al-enableAutoRelogin', '', Number.MAX_SAFE_INTEGER);
      ...
      darkModeAuto = true;
      setCookie('al-darkModeAuto', '1', Number.MAX_SAFE_INTEGER);
      menuOpacity = 0.9;
      setCookie('al-menuOpacity', menuOpacity, Number.MAX_SAFE_INTEGER);
  ...
  function exportConfig() {
    try {
      var config = {
        ...
        statusTextBrightness: statusTextBrightness,
        ...
        enableAutoRelogin: enableAutoRelogin,
      };
      ...
      if (backupStr) {
        ...
        Object.keys(cfg).forEach(function (k) {
          try {
            ...
            setCookie(
              'al-' + k,
              typeof cfg[k] === 'object' ? JSON.stringify(cfg[k]) : cfg[k],
              Number.MAX_SAFE_INTEGER
            );
          } catch (e) {}
        });
  ...
  function createMenu() {
    ...
    sidebar.innerHTML =
      '<h4>设置分类</h4><ul><li><button id="al-tab-core">核心参数</button></li><li><button id="al-tab-ui">界面自定义</button></li><li><button id="al-tab-filter">过滤规则</button></li><li><button id="al-tab-adv">高级参数</button></li><li><button id="al-tab-logs">查看日志</button></li><li><button id="al-tab-stats">性能统计</button></li><li><button id="al-tab-accounts">账号管理</button></li></ul>';
    ...
    function showTab(tab) {
      ...
      setTimeout(function () {
        ...
        if (tab === 'core') {
          content.innerHTML =
            '<h3>核心参数</h3><div class="al-card"><label>刷新频率 (秒): <input type="number" id="al-dur" value="' +
            duration +
            '" min="30" style="width: 80px; margin-left: 10px;"></label></div><div class="al-card"><label>刷新延迟 (秒): <input type="number" id="al-rdelay" value="' +
            refreshDelay +
            '" min="5" style="width: 80px; margin-left: 10px;"></label></div><div class="al-card"><label>点赞延迟 (秒): <input type="number" id="al-ldelay" value="' +
            likeDelay +
            '" min="3" style="width: 80px; margin-left: 10px;"></label></div><div class="al-card"><label>下滑动态数: <input type="number" id="al-scount" value="' +
            scrollCount +
            '" min="1" style="width: 80px; margin-left: 10px;"></label></div><div class="al-card"><label>每日点赞上限 (0无限): <input type="number" id="al-dailyLimit" value="' +
            dailyLimit +
            '" min="0" style="width: 80px; margin-left: 10px;"></label></div><div class="al-card"><label>智能点赞模式: <select id="al-smartLikeMode" style="width: 120px; margin-left: 10px;"><option value="normal" ' +
            (smartLikeMode === 'normal' ? 'selected' : '') +
            '>普通模式</option><option value="smart" ' +
            (smartLikeMode === 'smart' ? 'selected' : '') +
            '>智能模式</option><option value="aggressive" ' +
            (smartLikeMode === 'aggressive' ? 'selected' : '') +
            '>全面模式</option></select></label></div><div class="al-card"><label><input type="checkbox" id="al-select" ' +
            (select ? 'checked' : '') +
            '> 不点赞游戏转发内容</label></div><div class="al-card"><label><input type="checkbox" id="al-enableLoginCheck" ' +
            (enableLoginCheck ? 'checked' : '') +
            '> 启用登录状态检测</label></div>';
          content.innerHTML +=
            '<div class="al-card"><label><input type="checkbox" id="al-enableAutoRelogin" ' +
            (enableAutoRelogin ? 'checked' : '') +
            '> 启用自动重登录（过期时重定向）</label></div>'; // 新增复选框
        } else if (tab === 'ui') {
          content.innerHTML =
            '<h3>界面自定义</h3><div class="al-card"><label>主题: <select id="al-theme"><option value="default" ' +
            (theme === 'default' ? 'selected' : '') +
            '>默认</option><option value="tech" ' +
            (theme === 'tech' ? 'selected' : '') +
            '>科技蓝</option><option value="eco" ' +
            (theme === 'eco' ? 'selected' : '') +
            '>环保绿</option></select></label></div><div class="al-card"><label>主题色调调整 (0-360): <input type="number" id="al-themeHue" value="' +
            themeHue +
            '" min="0" max="360" style="width: 80px; margin-left: 10px;"></label></div><div class="al-card"><label>状态栏透明度 (0.1-1): <input type="number" id="al-statusOpacity" value="' +
            statusOpacity +
            '" min="0.1" max="1" step="0.1" style="width: 80px; margin-left: 10px;"></label></div><div class="al-card"><label>状态栏背景: <select id="al-statusBgColor" style="width: 200px; margin-left: 10px;"><option value="linear-gradient(to right, #333, #222)" ' +
            (statusBgColor === 'linear-gradient(to right, #333, #222)'
              ? 'selected'
              : '') +
            '>黑色渐变</option><option value="linear-gradient(to right, #f0f0f0, #e0e0e0)" ' +
            (statusBgColor === 'linear-gradient(to right, #f0f0f0, #e0e0e0)'
              ? 'selected'
              : '') +
            '>白色渐变</option><option value="linear-gradient(to right, #2196F3, #1976D2)" ' +
            (statusBgColor === 'linear-gradient(to right, #2196F3, #1976D2)'
              ? 'selected'
              : '') +
            '>蓝色渐变</option><option value="linear-gradient(to right, #4CAF50, #388E3C)" ' +
            (statusBgColor === 'linear-gradient(to right, #4CAF50, #388E3C)'
              ? 'selected'
              : '') +
            '>绿色渐变</option></select></label></div><div class="al-card"><label>状态栏文字颜色: <select id="al-statusTextColor" style="width: 200px; margin-left: 10px;"><option value="auto" ' +
            (statusTextColor === 'auto' ? 'selected' : '') +
            '>自动</option><option value="#fff" ' +
            (statusTextColor === '#fff' ? 'selected' : '') +
            '>白色</option><option value="#000" ' +
            (statusTextColor === '#000' ? 'selected' : '') +
            '>黑色</option><option value="#ddd" ' +
            (statusTextColor === '#ddd' ? 'selected' : '') +
            '>浅灰</option></select></label></div><div class="al-card"><label>状态栏文字亮度 (0.5-1.5): <input type="number" id="al-statusTextBrightness" value="' +
            statusTextBrightness +
            '" min="0.5" max="1.5" step="0.1" style="width: 80px; margin-left: 10px;"></label></div><div class="al-card"><label><input type="checkbox" id="al-darkModeAuto" ' +
            (darkModeAuto ? 'checked' : '') +
            '> 自动适配暗黑模式</label></div><div class="al-card"><label>控制面板透明度 (0.1-1): <input type="number" id="al-menuOpacity" value="' +
            menuOpacity +
            '" min="0.1" max="1" step="0.1" style="width: 80px; margin-left: 10px;"></label></div><div class="al-card"><label>控制面板背景: <select id="al-menuBgColor" style="width: 200px; margin-left: 10px;"><option value="linear-gradient(to bottom, #ffffff, #f0f0f0)" ' +
            (menuBgColor === 'linear-gradient(to bottom, #ffffff, #f0f0f0)'
              ? 'selected'
              : '') +
            '>白色渐变</option><option value="linear-gradient(to bottom, #333, #222)" ' +
            (menuBgColor === 'linear-gradient(to bottom, #333, #222)'
              ? 'selected'
              : '') +
            '>黑色渐变</option><option value="linear-gradient(to bottom, #2196F3, #1976D2)" ' +
            (menuBgColor === 'linear-gradient(to bottom, #2196F3, #1976D2)'
              ? 'selected'
              : '') +
            '>蓝色渐变</option><option value="linear-gradient(to bottom, #4CAF50, #388E3C)" ' +
            (menuBgColor === 'linear-gradient(to bottom, #4CAF50, #388E3C)'
              ? 'selected'
              : '') +
            '>绿色渐变</option></select></label></div>';
        } else if (tab === 'filter') {
          content.innerHTML =
            '<h3>过滤规则</h3><div class="al-card"><label>屏蔽用户 (QQ号,逗号分隔): <textarea id="al-blocked" style="width: 200px; height: 50px; margin-left: 10px;">' +
            blocked.join(',') +
            '</textarea></label></div><div class="al-card"><label>白名单用户 (QQ号,逗号分隔): <textarea id="al-whiteList" style="width: 200px; height: 50px; margin-left: 10px;">' +
            whiteList.join(',') +
            '</textarea></label></div><div class="al-card"><label>黑名单分组 (JSON): <textarea id="al-blockGroups" style="width: 200px; height: 100px; margin-left: 10px;">' +
            JSON.stringify(blockGroups) +
            '</textarea></label></div><div class="al-card"><label>动态关键词 (逗号分隔,支持正则): <textarea id="al-filterKeywords" style="width: 200px; height: 50px; margin-left: 10px;">' +
            filterKeywords.join(',') +
            '</textarea></label></div><div class="al-card"><label>过滤模式: <select id="al-filterMode"><option value="block" ' +
            (filterMode === 'block' ? 'selected' : '') +
            '>屏蔽关键词</option><option value="allow" ' +
            (filterMode === 'allow' ? 'selected' : '') +
            '>仅允许关键词</option></select></label></div><div class="al-card"><button id="al-import-block" style="background: #2196F3; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer;">导入黑名单</button> <input type="file" id="al-file-input" style="display:none;"><button id="al-export-block" style="background: #673AB7; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer;">导出黑名单</button></div>';
          // 添加事件监听器在这里，确保元素存在
          document
            .getElementById('al-import-block')
            .addEventListener('click', function () {
              document.getElementById('al-file-input').click();
            });
          document
            .getElementById('al-file-input')
          ...
          document
            .getElementById('al-export-block')
            .addEventListener('click', function () {
              let data = {
                blocked: blocked,
                blockGroups: blockGroups,
          ...
          content.innerHTML =
            '<h3>高级参数</h3><div class="al-card"><label>最大重试次数: <input type="number" id="al-maxRetries" value="' +
            maxRetries +
            '" min="1" style="width: 80px; margin-left: 10px;"></label></div><div class="al-card"><label>滚动步长百分比 (0.1-1): <input type="number" id="al-scrollStepPercent" value="' +
            scrollStepPercent +
            '" min="0.1" max="1" step="0.1" style="width: 80px; margin-left: 10px;"></label></div><div class="al-card"><label>初始延迟 (毫秒): <input type="number" id="al-initialDelay" value="' +
            initialDelay +
            '" min="1000" style="width: 80px; margin-left: 10px;"></label></div><div class="al-card"><label>随机延迟最小 (秒): <input type="number" id="al-randomDelayMin" value="' +
            randomDelayMin +
            '" min="0" style="width: 80px; margin-left: 10px;"></label></div><div class="al-card"><label>随机延迟最大 (秒): <input type="number" id="al-randomDelayMax" value="' +
            randomDelayMax +
            '" min="0" style="width: 80px; margin-left: 10px;"></label></div><div class="al-card"><label>日志级别: <select id="al-logLevel" style="width: 100px; margin-left: 10px;"><option value="INFO" ' +
            (logLevel === 'INFO' ? 'selected' : '') +
            '>INFO</option><option value="WARN" ' +
            (logLevel === 'WARN' ? 'selected' : '') +
            '>WARN</option><option value="ERROR" ' +
            (logLevel === 'ERROR' ? 'selected' : '') +
            '>ERROR</option></select></label></div><div class="al-card"><label><input type="checkbox" id="al-enableNotifications" ' +
            (enableNotifications ? 'checked' : '') +
            '> 启用浏览器通知</label></div>';
        } else if (tab === 'logs') {
          content.innerHTML =
            '<h3>系统日志</h3><div id="al-log-list" style="height: 300px; overflow: auto; border: 1px solid #ddd; padding: 10px; background: #f9f9f9; font-family: monospace; white-space: pre-wrap;">' +
            (logs[currentAccount] || []).join('\n') +
          ...
          document
            .getElementById('al-export-logs')
            .addEventListener('click', function () {
              download(
                'al-logs-' + currentAccount + '.json',
                JSON.stringify(logs[currentAccount] || [])
              );
            });
        } else if (tab === 'stats') {
          let accStats = stats[currentAccount] || {
            likes: 0,
            skips: 0,
            errors: 0,
          };
          ...
          let successRate =
            total > 0 ? Math.round((successSum / total) * 100) + '%' : 'N/A';
    ...
    `;
          ...
          document
            .getElementById('al-export-stats')
            .addEventListener('click', function () {
              ...
              const exportFileDefaultName =
                'qzone-stats-' + currentAccount + '.json';
          ...
          content.innerHTML =
            '<h3>账号管理</h3><div class="al-card"><label>当前账号: ' +
            currentAccount +
            '</label></div><div class="al-card"><label>切换账号 (输入QQ号): <input type="text" id="al-switchAcc" style="width: 150px; margin-left: 10px;"></label><button id="al-switch" style="background: #2196F3; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer; margin-left: 10px;">切换</button></div><div class="al-card">可用账号: ' +
            accountList +
            '</div>';
          document
            .getElementById('al-switch')
            .addEventListener('click', function () {
    ...
    // 标签切换事件
    document
      .getElementById('al-tab-core')
      .addEventListener('click', function () {
        showTab('core');
      });
    document.getElementById('al-tab-ui').addEventListener('click', function () {
      showTab('ui');
    });
    document
      .getElementById('al-tab-filter')
      .addEventListener('click', function () {
        showTab('filter');
      });
    document
      .getElementById('al-tab-adv')
      .addEventListener('click', function () {
        showTab('adv');
      });
    document
      .getElementById('al-tab-logs')
      .addEventListener('click', function () {
        showTab('logs');
      });
    document
      .getElementById('al-tab-stats')
      .addEventListener('click', function () {
        showTab('stats');
      });
    document
      .getElementById('al-tab-accounts')
      .addEventListener('click', function () {
        showTab('accounts');
      });
    ...
    document.getElementById('al-save').addEventListener('click', function () {
      try {
        // 获取控制面板中的值
        if (document.getElementById('al-dur'))
          duration =
            parseInt(document.getElementById('al-dur').value) || duration;
        if (document.getElementById('al-rdelay'))
          refreshDelay =
            parseInt(document.getElementById('al-rdelay').value) ||
            refreshDelay;
        if (document.getElementById('al-ldelay'))
          likeDelay =
            parseInt(document.getElementById('al-ldelay').value) || likeDelay;
        if (document.getElementById('al-scount'))
          scrollCount =
            parseInt(document.getElementById('al-scount').value) || scrollCount;
        if (document.getElementById('al-dailyLimit'))
          dailyLimit =
            parseInt(document.getElementById('al-dailyLimit').value) ||
            dailyLimit;
        if (document.getElementById('al-select'))
          select = document.getElementById('al-select').checked;
        if (document.getElementById('al-theme'))
          theme = document.getElementById('al-theme').value;
        if (document.getElementById('al-themeHue'))
          themeHue =
            parseInt(document.getElementById('al-themeHue').value) || themeHue;
        if (document.getElementById('al-statusOpacity'))
          statusOpacity =
            parseFloat(document.getElementById('al-statusOpacity').value) ||
            statusOpacity;
        if (document.getElementById('al-statusBgColor'))
          statusBgColor = document.getElementById('al-statusBgColor').value;
        if (document.getElementById('al-statusTextColor'))
          statusTextColor = document.getElementById('al-statusTextColor').value;
        if (document.getElementById('al-statusTextBrightness'))
          statusTextBrightness =
            parseFloat(
              document.getElementById('al-statusTextBrightness').value
            ) || statusTextBrightness;
        if (document.getElementById('al-darkModeAuto'))
          darkModeAuto = document.getElementById('al-darkModeAuto').checked;
        if (document.getElementById('al-menuOpacity'))
          menuOpacity =
            parseFloat(document.getElementById('al-menuOpacity').value) ||
            menuOpacity;
        if (document.getElementById('al-menuBgColor'))
          menuBgColor = document.getElementById('al-menuBgColor').value;
        if (document.getElementById('al-blocked'))
          blocked = document
            .getElementById('al-blocked')
            .value.split(',')
            .map((s) => s.trim())
            .filter((s) => s);
        if (document.getElementById('al-whiteList'))
          whiteList = document
            .getElementById('al-whiteList')
            .value.split(',')
            .map((s) => s.trim())
            .filter((s) => s);
        if (document.getElementById('al-blockGroups'))
          blockGroups =
            safeJsonParse(document.getElementById('al-blockGroups').value) ||
            blockGroups;
        if (document.getElementById('al-filterKeywords'))
          filterKeywords = document
            .getElementById('al-filterKeywords')
            .value.split(',')
            .map((s) => s.trim())
            .filter((s) => s);
        if (document.getElementById('al-filterMode'))
          filterMode = document.getElementById('al-filterMode').value;
        if (document.getElementById('al-maxRetries'))
          maxRetries =
            parseInt(document.getElementById('al-maxRetries').value) ||
            maxRetries;
        if (document.getElementById('al-scrollStepPercent'))
          scrollStepPercent =
            parseFloat(document.getElementById('al-scrollStepPercent').value) ||
            scrollStepPercent;
        if (document.getElementById('al-initialDelay'))
          initialDelay =
            parseInt(document.getElementById('al-initialDelay').value) ||
            initialDelay;
        if (document.getElementById('al-randomDelayMin'))
          randomDelayMin =
            parseInt(document.getElementById('al-randomDelayMin').value) ||
            randomDelayMin;
        if (document.getElementById('al-randomDelayMax'))
          randomDelayMax =
            parseInt(document.getElementById('al-randomDelayMax').value) ||
            randomDelayMax;
        if (document.getElementById('al-logLevel'))
          logLevel = document.getElementById('al-logLevel').value;
        if (document.getElementById('al-enableNotifications'))
          enableNotifications = document.getElementById(
            'al-enableNotifications'
          ).checked;
        if (document.getElementById('al-enableLoginCheck'))
          enableLoginCheck = document.getElementById(
            'al-enableLoginCheck'
          ).checked;
        if (document.getElementById('al-enableAutoRelogin'))
          enableAutoRelogin = document.getElementById(
            'al-enableAutoRelogin'
          ).checked;
        if (document.getElementById('al-smartLikeMode'))
          smartLikeMode = document.getElementById('al-smartLikeMode').value;
    ...
    document.getElementById('al-reset').addEventListener('click', function () {
      ...
      showTab(
        document.querySelector('[id^="al-tab-"]').id.replace('al-tab-', '')
      );
    });
  ...
  function createFloatButton() {
    ...
    btn.addEventListener('click', function () {
      ...
      if (typeof window.showMenu === 'function') {
        ...
        typeof unsafeWindow.showMenu === 'function'
      ) {
  ...
  function saveAccountConfig() {
    accounts[currentAccount] = {
      ...
      enableLoginCheck: enableLoginCheck,
      enableAutoRelogin: enableAutoRelogin,
      stats: stats[currentAccount],
      logs: logs[currentAccount],
    };
  ...
  function createStatusBar() {
    ...
    const optimalTextColor =
      statusTextColor === 'auto'
        ? getOptimalTextColor(statusBgColor)
        : statusTextColor;
  ...
  function updateStatusBar(message) {
    ...
    if (currentTask && taskStartTime && taskDuration > 0) {
      ...
      progressPercent = Math.min(
        100,
        Math.floor((elapsed / taskDuration) * 100)
      );
      ...
      let nextRefreshSeconds = Math.max(
        0,
        Math.ceil((nextTime - Date.now()) / 1000)
      );
    ...
    // 构建详细的状态信息
    const dailyProgress =
      dailyLimit > 0 ? `${dailyCount}/${dailyLimit}` : `${dailyCount}`;
    const accStatsForRate = stats[currentAccount] || {
      likes: 0,
      skips: 0,
      errors: 0,
    };
    const totalActions =
      accStatsForRate.likes + accStatsForRate.skips + accStatsForRate.errors;
    const successRate =
      totalActions > 0
        ? Math.round(
            ((accStatsForRate.likes + accStatsForRate.skips) / totalActions) *
              100
          )
        : 100;
  ...
  function removeMeRelatedMenu() {
    let meTab =
      document.getElementById('tab_menu_me') ||
  ...
  function isInFriendFeedPage() {
    const hasLikeButtons =
      document.querySelectorAll('.qz_like_btn_v3').length > 0;
  ...
  function goToFriendFeed() {
    try {
      ...
      let friendTab =
        document.getElementById('tab_menu_friend') ||
  ...
  function safeLike() {
    try {
      const now = Date.now();
      if (isPaused || now - lastLikeTime < 1000) {
        // 防重复触发，1秒冷却
      ...
      likeDebounce = setTimeout(function () {
        ...
        Array.from(btns).forEach(function (btn, index) {
          ...
          const user =
            users[index] && users[index].getAttribute('link')
              ? users[index].getAttribute('link').replace('nameCard_', '')
              : '';
          ...
          filterKeywords.forEach(function (kw) {
            try {
              ...
              if (content.includes(kw)) {
                ...
                log(
                  'INFO',
                  '关键词包含: ' + kw + ' 在动态 ' + (index + 1) + ' 中找到'
                );
              }
            }
          });
          if (
            (filterMode === 'block' && matchesKeyword) ||
            (filterMode === 'allow' && !matchesKeyword)
          ) {
            skipped++;
            updateStats('skips');
        ...
        taskDuration =
          effectiveLikes * (likeDelay + (randomDelayMax - randomDelayMin) / 2) +
          1;
        nextTask = '模拟滚动或等待刷新';
        nextTime = Math.max(nextTime, Date.now() + taskDuration * 1000 + 5000);
        updateStatusBar(
          '开始点赞 (需赞: ' + effectiveLikes + ', 跳过: ' + skipped + ')'
        );
        log(
          'INFO',
          '开始点赞 (需赞: ' + effectiveLikes + ', 跳过: ' + skipped + ')'
        ...
        toLike.forEach(function (item, idx) {
          ...
          setTimeout(function () {
            if (isPaused || (dailyLimit > 0 && dailyCount >= dailyLimit))
              return;
    ...
    try {
      ...
      function scrollLoop(i) {
        ...
        let loadMoreBtn =
          ...
          document.querySelector('a[title="加载更多"]');
        if (loadMoreBtn) {
          loadMoreBtn.click();
